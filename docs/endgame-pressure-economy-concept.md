...
6. `Expansion: Governance & Belief`
- Ветки технологий, политики, религия/идеология как полноценная метасистема.
- Новый слой общественного давления и легитимности власти.
...
- апгрейд без повторной покупки базы.
2. `Year 1 Pass`
...
## 9.7 Expansion DLC: Governance & Belief (детализация)
## 9.7.1 Упаковка
1. Бесплатный патч в базу:
- инфраструктура систем (интерфейсы, UI-слоты, базовые индексы).
- 1 базовая ветка технологий и 1 базовый политический режим для всех игроков.
2. Платная часть expansion:
- 4-6 полноценных технологических веток.
- 8-12 политических доктрин с сильными trade-off.
- система религии/идеологии с фракциями и социальным трением.
- уникальные кризисы и сценарии, связанные с управлением обществом.

## 9.7.2 Ветки технологий (пример)
- `Industrial Sovereignty`: производство, логистика, maintenance automation.
- `Social Welfare`: здравоохранение, жилье, социальная стабильность.
- `Civic Security`: порядок, ЧС-реакция, превентивные меры.
- `Green Transition`: экология, энергоэффективность, климатическая устойчивость.
- `Data Governance`: аналитика, прогноз кризисов, оптимизация решений.

## 9.7.3 Политики
- Налоговый профиль (низкие/средние/высокие налоги).
- Приоритет бюджета (инфраструктура/социалка/безопасность/наука).
- Уровень регулирования (мягкий/сбалансированный/жесткий).
- Трудовая политика (производительность vs благополучие).

Каждая политика:
- имеет немедленный эффект;
- имеет отложенный риск;
- взаимодействует с pressure director и citizen requests.

## 9.7.4 Религия/идеология (дизайн безопасного уровня)
Рекомендуется абстрактная модель без ссылок на реальные религии и партии:
- блоки мировоззрения: `Traditional`, `Progressive`, `Technocratic`, `Collective`, `Market`.
- у каждого блока:
  - ожидания к городу;
  - чувствительность к кризисам;
  - реакция на налоги и реформы;
  - потенциал радикализации при провале запросов.

Эффект:
- добавляется слой легитимности власти;
- кризисы получают социальную фазу (не только инфраструктурную);
- появляется "политический эндгейм", а не только экономический.

## 9.7.5 Монетизационная упаковка expansion
- Рекомендуемая цена: `$14.99-$19.99`.
- Attach target: `12-20%` от базы в первые 6 месяцев после релиза.
- Продавать как "новый способ играть", а не как power boost.
...
Этап 4 (expansion-ready foundation):
1. Ввести extension points для DLC (tick/pressure/citizen/ui/save hooks).
2. Перевести текущий контент на manifest-driven модель.
3. Подготовить namespaced save для модулей.

Этап 5 (Governance & Belief):
1. Добавить `TechTreeSystem`.
2. Добавить `PolicyEngine`.
3. Добавить `BeliefDynamicsSystem` и индекс легитимности.
4. Добавить социальные кризисы как отдельный слой pressure director.
